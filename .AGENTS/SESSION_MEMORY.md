...
- `CORE_MIGRATION_STATUS.md` — Per-file blocker analysis for core abstraction
- `CORE_PLAN.md` — Platform abstraction roadmap and naming conventions
...
| Entity registration | MINOR_CHANGE | 1 file (CustomNpcs.java) | Package rename |
| Block/Item registration (GameRegistry) | IDENTICAL | 18 files (62 calls in CustomItems.java) | Same API |
...
| IInventory → IItemHandler (Capabilities) | BREAKING_CHANGE | 40+ containers | New system alongside old |
...
| **ModStitch** (github.com/isXander/modstitch) | Unifies RFG + ForgeGradle + NeoGradle in one build | THEORETICAL — needs validation |
...
| 1.7.10→1.12.2 gap | Treated as equivalent to other gaps | Mostly IDENTICAL or MINOR_CHANGE (524 package renames + 1 breaking: packets) | **1.12.2 is a mechanical port — much easier than expected** |
...
The key insight: **1.12.2 is so close to 1.7.10 that most of the work is mechanical renaming (524 files of cpw.mods.fml → net.minecraftforge.fml).** This means:

### Track A: Core Abstraction (enables ALL versions)
1. Finish mc1710 shadow files for migrated controllers
2. Implement `IUser` usage in core (unblocks Tier 2: ~15 files)
3. Implement `IStack` / inventory abstraction (unblocks Tier 1: ~8 files)
4. Dialog cluster → Quest cluster → PlayerData migration to core
5. Packet DTO abstraction in core
6. Command abstraction in core

### Track B: 1.12.2 Port (CAN START IMMEDIATELY — mostly mechanical)
1. Copy mc1710/ → mc1122/
2. Mass-rename cpw.mods.fml → net.minecraftforge.fml (524 files)
3. Update entity registration (minor API change)
4. Refactor packet system (SimpleNetworkWrapper → SimpleChannel — the ONE breaking change)
5. Set up RetroFuturaGradle for 1.12.2 (confirmed: RFG supports 1.12.2)
6. Test and iterate
...
### 1.12.2 — "Bridge Build" (EASIEST — mostly mechanical port)
...
5. **Don't try to abstract scripting, abilities, entities, or recipes** — Tier 4-5, rewrite per-version
6. **1.12.2 is much easier than you think** — 95% of APIs are identical/renamed, only packets truly break
7. **Composite builds, not separate wrappers** — one Gradle wrapper, `includeBuild` per version leaf
