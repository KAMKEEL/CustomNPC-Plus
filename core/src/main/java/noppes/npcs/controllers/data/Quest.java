...
public class Quest implements ICompatibilty, IQuest {
    ...
    public void readNBTPartial(INbt compound) {
        ...
        partyOptions.readFromNBT(new NBTWrapper(compound.getCompoundTag("PartyOptions")));
        profileOptions.readFromNBT(new NBTWrapper(compound.getCompoundTag("ProfileOptions")));
    ...
    public INbt writeToNBTPartial(INbt compound) {
        ...
        compound.setTag("QuestFactionPoints", factionOptions.writeToNBT(new INbt()));
        compound.setTag("PartyOptions", ((NBTWrapper) partyOptions.writeToNBT()).getMCTag());
        compound.setTag("ProfileOptions", ((NBTWrapper) profileOptions.writeToNBT()).getMCTag());
    ...
    public boolean instantComplete(IPlayer player, QuestData data) {
        if (completion == EnumQuestCompletion.Instant && NoppesUtilPlayer.questCompletion((IPlayer) player, data.quest.id)) {
            QuestCompletionPacket.sendQuestComplete((IPlayer) player, data.quest.writeToNBT(new INbt()));
            return true;
        }
        return false;
    }
    ...
    public Quest copy() {
        ...
        quest.readNBT(this.writeToNBT(new INbt()));
        return quest;
    }
    ...
    public long calculateRemainingCooldown(IPlayer player, long questTime) {
        switch (repeat) {
            case MCDAILY: {
    ...
    public long getTimeUntilRepeatQuestData(IPlayer player, IPlayerQuestData questData) {
    ...
    public IQuestObjective[] getObjectives(IPlayer player) {
        if (!player.hasActiveQuest(this.id)) {
            ...
            return this.questInterface.getObjectives((IPlayer) player.getMCEntity());
        }
    }
