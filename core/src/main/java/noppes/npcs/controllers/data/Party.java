...
public class Party implements IParty {
    ...
    public boolean setLeader(IPlayer player) {
        UUID uuid = player.getUniqueID();
    ...
    public boolean validateQuest(int questID, boolean sendLeaderMessages) {
        ...
        IPlayer leader = PlayerDataController.getPlayerFromUUID(partyLeader);
        if (leader == null) {
            return false;
        }
    ...
    private boolean isValidLeaderQuest(IPlayer leader, int questID) {
        IPlayerQuestData questData = PlayerData.get(leader).getQuestData();
        return questData != null && questData.hasActiveQuest(questID);
    }

    private boolean arePlayersValid(IPlayer leader, EnumPartyRequirements requirements, IQuest quest, boolean sendLeaderMessages) {
        boolean allowQuest = true;
        for (UUID playerUUID : partyMembers.keySet()) {
            IPlayer player = PlayerDataController.getPlayerFromUUID(playerUUID);
            if (player != null) {
                PlayerData playerData = PlayerData.get(player);
                if (playerData != null) {
    ...
    private void sendInfoMessage(IPlayer player, String message, boolean send) {
        if (!send)
            return;

        player.addChatMessage(new ITextComponent(String.format("\u00A7c%s", message)));
    }
    ...
    public void readFromNBT(INbt compound) {
        ...
        INbtList list = compound.getTagList("PartyMembers", 10);
        for (int i = 0; i < list.tagCount(); i++) {
            INbt tagCompound = list.getCompoundTagAt(i);
    ...
    public INbt writeToNBT() {
        ...
        if (this.getQuest() != null) {
            ...
            compound.setTag("QuestNBT", quest.writeToNBT(new INbt()));
        }

        INbtList list = new INbtList();
        for (UUID uuid : this.partyOrder) {
            INbt uuidCompound = new INbt();
    ...
    public void readClientNBT(INbt compound) {
        this.currentQuestID = compound.getInteger("PartyQuestID");
        this.currentQuestName = compound.getString("PartyQuestName");
        this.friendlyFire = compound.getBoolean("FriendlyFire");
