...
public class PlayerTradeData implements IPlayerTradeData {
    ...
    public void readFromNBT(INbt compound) {
        INbt tradeData = compound.getCompoundTag(NBT_KEY);
        ...
        if (tradeData.hasKey("AuctionClaims")) {
            INbtList claimsList = tradeData.getTagList("AuctionClaims", 10);
    ...
    public void writeToNBT(INbt compound) {
        ...
        // Claims
        INbtList claimsList = new INbtList();
        for (AuctionClaim claim : claims) {
            if (!claim.claimed) {
                claimsList.appendTag(claim.writeToNBT(new INbt()));
            }
        }
