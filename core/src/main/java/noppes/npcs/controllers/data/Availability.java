...
public class Availability implements ICompatibilty, IAvailability {
    ...
    @ClientOnly
    public List<String> isAvailableText(IPlayer player) {
        ...
        if (daytime == EnumDayTime.Day) {
            long time = player.worldObj.getWorldTime() % 24000;
            if (time > 12000) {
                errors.add(PlatformServiceHolder.get().translateToLocal("availability.error.day"));
            }
        } else if (daytime == EnumDayTime.Night) {
            long time = player.worldObj.getWorldTime() % 24000;
            if (time < 12000) {
                errors.add(PlatformServiceHolder.get().translateToLocal("availability.error.night"));
            }
        }

        // Check dialog conditions.
        if (!dialogAvailable(dialogId, dialogAvailable, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.dialog", dialogAvailable.toString(), dialogId));
        }
        if (!dialogAvailable(dialog2Id, dialog2Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.dialog", dialog2Available.toString(), dialog2Id));
        }
        if (!dialogAvailable(dialog3Id, dialog3Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.dialog", dialog3Available.toString(), dialog3Id));
        }
        if (!dialogAvailable(dialog4Id, dialog4Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.dialog", dialog4Available.toString(), dialog4Id));
        }

        // Check quest conditions.
        if (!questAvailable(questId, questAvailable, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.quest", questAvailable.toString(), questId));
        }
        if (!questAvailable(quest2Id, quest2Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.quest", quest2Available.toString(), quest2Id));
        }
        if (!questAvailable(quest3Id, quest3Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.quest", quest3Available.toString(), quest3Id));
        }
        if (!questAvailable(quest4Id, quest4Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.quest", quest4Available.toString(), quest4Id));
        }

        // Check faction conditions.
        if (!factionAvailable(factionId, factionStance, factionAvailable, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.faction", factionAvailable.toString(), factionStance.toString(), factionId));
        }
        if (!factionAvailable(faction2Id, faction2Stance, faction2Available, player)) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.faction", faction2Available.toString(), faction2Stance.toString(), faction2Id));
        }
        if (player.experienceLevel < minPlayerLevel) {
            errors.add(PlatformServiceHolder.get().translateToLocalFormatted("availability.error.xp", minPlayerLevel));
        }
        return errors;
    }

    private boolean factionAvailable(int id, EnumAvailabilityFaction stance, EnumAvailabilityFactionType available, IPlayer player) {
    ...
    public boolean dialogAvailable(int id, EnumAvailabilityDialog en, IPlayer player) {
        if (player == null)
            return false;
    ...
    public boolean questAvailable(int id, EnumAvailabilityQuest en, IPlayer player) {
    ...
    public void setDaytime(int type) {
        this.daytime = EnumDayTime.values()[ValueUtil.clampInt(type, 0, 2)];
    }
    ...
    public void setDialog(int i, int id, int type) {
        if (i < 0 || i > 3) {
            ...
            EnumAvailabilityDialog e = EnumAvailabilityDialog.values()[ValueUtil.clampInt(type, 0, 2)];
            if (i == 0) {
                this.dialogId = id;
    ...
    public void setQuest(int i, int id, int type) {
        if (i < 0 || i > 3) {
            ...
            EnumAvailabilityQuest e = EnumAvailabilityQuest.values()[ValueUtil.clampInt(type, 0, 5)];
            if (i == 0) {
                this.questId = id;
    ...
    public void setFaction(int i, int id, int type, int stance) {
        if (i < 0 || i > 1) {
            ...
            EnumAvailabilityFactionType e = EnumAvailabilityFactionType.values()[ValueUtil.clampInt(type, 0, 2)];
            EnumAvailabilityFaction ee = EnumAvailabilityFaction.values()[ValueUtil.clampInt(stance, 0, 2)];
            if (i == 0) {
                this.factionId = id;
