...
public class PlayerDataScript extends MultiScriptHandler {
    ...
    public PlayerDataScript(IPlayer player) {
        if (player != null) {
            this.player = player;
        }
    }
    ...
    public boolean isEnabled() {
        return CustomNpcs.proxy.isGlobalPlayerScripts() && ScriptController.Instance.playerScripts.enabled && ScriptController.HasStart && (this.player == null || !this.player.worldObj.isRemote);
    }
            ...
            if (ScriptController.Instance.lastLoaded > lastInited || ScriptController.Instance.lastPlayerUpdate > lastPlayerUpdate) {
                ...
                if (player != null) {
                    ...
                    for (IScriptUnit script : ScriptController.Instance.playerScripts.scripts) {
                        ...
                        INbt nbt = script.writeToNBT(new INbt());
