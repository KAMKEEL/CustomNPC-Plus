...
public class SpawnData extends WeightedRandom.Item implements INaturalSpawn {
    ...
    public HashMap<Integer, INbt> spawnCompounds = new HashMap<>();
    ...
    public INbt writeNBT(INbt compound) {
        ...
        Set<Map.Entry<Integer, INbt>> entries = this.spawnCompounds.entrySet();
        for (Map.Entry<Integer, INbt> entry : entries) {
            compound.setTag("SpawnCompound" + entry.getKey(), entry.getValue() != null ? entry.getValue() : new INbt());
        }
    ...
    public void setEntity(IEntity IEntity, int slot) {
        INbt compound = new INbt();
        if (IEntity != null && !IEntity.getMCEntity().writeToNBTOptional(compound)) {
            throw new CustomNPCsException("IEntity could not be written to NBT");
        } else {
            this.spawnCompounds.put(slot, compound);
        }
    }

    public IEntity getEntity(IWorld IWorld, int slot) {
        if (!this.spawnCompounds.containsKey(slot)) {
            return null;
        }
        try {
            IEntity IEntity = EntityList.createEntityFromNBT(this.spawnCompounds.get(slot), IWorld.getMCWorld());
            return NpcAPI.Instance().getIEntity(IEntity);
        } catch (Exception e) {
            throw new CustomNPCsException("Error creating IEntity from spawn data:\n" + e.getMessage());
        }
    }
