...
public class PlayerAbilityData extends AbstractDataAbilities implements IPlayerAbilityData {
     ...
     */
    private transient IEntityLivingBase currentTarget;
    ...
    /**
     * IWorld time when the last ability was activated (for double-press cancel detection).
     */
    ...
    @Override
    protected IEntityLivingBase getEntity() {
        return playerData.player;
    }

    @Override
    protected IEntityLivingBase getTarget() {
        return currentTarget;
    }

    @Override
    protected long getWorldTime() {
        IPlayer player = playerData.player;
        return player != null ? player.worldObj.getTotalWorldTime() : 0;
    }

    @Override
    protected void spawnTelegraph(Ability ability, IEntityLivingBase target) {
        IPlayer player = playerData.player;
    ...
    protected void clearAnimationData() {
        ...
        // IEntity hasn't migrated to the new chunk yet — so the AABB search centered
     ...
     */
    public void tick(IPlayer player) {
     ...
     */
    public boolean activateAbility(IPlayer player) {
     ...
     * @return true if the ability was started
     */
    public boolean activateAbility(IPlayer player, String key) {
     ...
     * Delegates to {@link SyncController#syncAbilities(IPlayer)}.
     */
    public void syncToClient() {
        IPlayer player = playerData.player;
        if (player instanceof IPlayer) {
            SyncController.syncAbilities((IPlayer) player);
        }
    }

    /**
     * Lightweight sync of cooldown state only to the client.
     * Delegates to {@link SyncController#syncAbilityCooldowns(IPlayer)}.
     */
    public void syncCooldownToClient() {
        IPlayer player = playerData.player;
        if (player instanceof IPlayer) {
            SyncController.syncAbilityCooldowns((IPlayer) player);
        }
    }
     ...
     */
    public boolean isOnCooldown(IPlayer player) {
     ...
     */
    public boolean isOnCooldown(String key, IPlayer player) {
    ...
    public long getRemainingCooldown(IPlayer player) {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // IEntity RECONSTRUCTION RESET
    // ═══════════════════════════════════════════════════════════════════

    /**
     * Handle IEntity reconstruction events (dimension change, death/respawn, etc.).
     * Cleans up any executing ability state without firing script events (those were
     * already fired on the original IEntity), clears transient runtime state, and
     * re-syncs ability data to the client on the new IEntity.
     * <p>
     * This should be called whenever the player's IEntity object is recreated by Forge,
     * since the client-side IEntity loses all synced ability state.
     *
    ...
    public void onEntityReconstructed(boolean clearCooldowns, boolean clearToggles) {
        // Clean up any executing ability silently (no script events — already fired on old IEntity)
        ...
        // (not gated by lastSyncedFlags != 0) because the NEW IEntity's client
        // may have stale lock state from before the reconstruction.
        lastSyncedFlags = 0;
        IPlayer player = playerData.player;
        if (player instanceof IPlayer) {
            PlayerAbilityStatePacket.sendToPlayer((IPlayer) player, (byte) 0, 0, 0);
        }
     ...
     * rolls cooldown) before the player IEntity goes away, then clears all transient state.
     ...
     */
    public float onDamage(IDamageSource source, float amount) {
        if (currentAbility == null || !currentAbility.isExecuting()) {
            return amount;
        }
     ...
     * Enforces locked rotation values on the player IEntity.
     */
    private void applyRotationControl(IPlayer player) {
     ...
     */
    private void applyPositionLock(IPlayer player) {
     ...
     */
    private void syncAbilityStateIfNeeded(IPlayer player) {
        if (!(player instanceof IPlayer)) return;
        byte flags = getAbilityStateFlags();
        if (flags != lastSyncedFlags) {
            lastSyncedFlags = flags;
            PlayerAbilityStatePacket.sendToPlayer((IPlayer) player, flags, lockedYaw, lockedPitch);
        }
    }
     ...
     */
    private void syncAbilityStateClear(IPlayer player) {
        if (!(player instanceof IPlayer)) return;
    ...
    public void readFromNBT(INbt compound) {
        ...
        if (compound.hasKey("PlayerAbilities")) {
            INbtList list = compound.getTagList("PlayerAbilities", 8); // 8 = string tag
        ...
        if (compound.hasKey("ActiveToggles")) {
            INbtList toggleNbt = compound.getTagList("ActiveToggles", 10); // 10 = TAG_COMPOUND
            for (int i = 0; i < toggleNbt.tagCount(); i++) {
                INbt entry = toggleNbt.getCompoundTagAt(i);
    ...
    private void validateUnlockedAbilities() {
        ...
        if (playerData == null || !(playerData.player instanceof IPlayer)) return;
