...
public class PlayerQuestData implements IPlayerQuestData {
    ...
    public void loadNBTData(INbt mainCompound) {
        if (mainCompound == null)
            return;
        INbt compound = mainCompound.getCompoundTag("QuestData");

        INbtList list = compound.getTagList("CompletedQuests", 10);
        if (list != null) {
            HashMap<Integer, Long> finishedQuests = new HashMap<Integer, Long>();
            for (int i = 0; i < list.tagCount(); i++) {
                INbt INbt = list.getCompoundTagAt(i);
                finishedQuests.put(INbt.getInteger("Quest"), INbt.getLong("Date"));
            }
            this.finishedQuests = finishedQuests;
        }

        INbtList list2 = compound.getTagList("ActiveQuests", 10);
        if (list2 != null) {
            HashMap<Integer, QuestData> activeQuests = new HashMap<Integer, QuestData>();
            for (int i = 0; i < list2.tagCount(); i++) {
                INbt INbt = list2.getCompoundTagAt(i);
                int id = INbt.getInteger("Quest");
                Quest quest = QuestController.Instance.quests.get(id);
                if (quest == null)
                    continue;
                QuestData data = new QuestData(quest);
                data.readEntityFromNBT(INbt);
    ...
    public void saveNBTData(INbt maincompound) {
        INbt compound = new INbt();
        INbtList list = new INbtList();
        for (int quest : finishedQuests.keySet()) {
            INbt INbt = new INbt();
            INbt.setInteger("Quest", quest);
            INbt.setLong("Date", finishedQuests.get(quest));
            list.appendTag(INbt);
        }

        compound.setTag("CompletedQuests", list);

        INbtList list2 = new INbtList();
        for (int quest : activeQuests.keySet()) {
            INbt INbt = new INbt();
            INbt.setInteger("Quest", quest);
            activeQuests.get(quest).writeEntityToNBT(INbt);
            list2.appendTag(INbt);
        }
    ...
    public QuestData getQuestCompletion(IPlayer player, EntityNPCInterface npc) {
    ...
    public Party getPartyQuestCompletion(IPlayer player, EntityNPCInterface npc) {
        PlayerData playerData = PlayerData.get(player);
        if (playerData != null) {
    ...
    public boolean checkQuestCompletion(PlayerData playerData, EnumQuestType type) {
        ...
        for (QuestData data : activeQuestValues) {
            ...
            if (this.trackedQuest != null && data.quest.getId() == this.trackedQuest.getId()) {
                NoppesUtilPlayer.sendTrackedQuestData((IPlayer) player);
            }
        }
    ...
    public void trackQuest(IQuest quest) {
        if (this.trackedQuest == null || quest.getId() != this.trackedQuest.getId()) {
            ...
            NoppesUtilPlayer.sendTrackedQuestData((IPlayer) this.parent.player);
        }
    }
    ...
    public void trackParty(IParty party) {
        ...
        if (this.trackedQuest == null || quest.getId() != this.trackedQuest.getId()) {
            this.trackedQuest = quest;
            NoppesUtilPlayer.sendPartyTrackedQuestData((IPlayer) this.parent.player, (Party) party);
        }
    }
    ...
    public void untrackQuest() {
        if (this.trackedQuest != null) {
            ...
            PacketHandler.Instance.sendToPlayer(new OverlayQuestTrackingPacket(new INbt()), (IPlayer) this.parent.player);
        }
    }
    ...
    public void startQuest(int id) {
        ...
        AchievementPacket.sendAchievement((IPlayer) parent.player, false, "quest.newquest", quest.title);
        ChatAlertPacket.sendChatAlert((IPlayer) parent.player, "quest.newquest", ": ", quest.title);
