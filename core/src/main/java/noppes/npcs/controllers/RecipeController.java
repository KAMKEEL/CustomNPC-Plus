...
public class RecipeController implements IRecipeHandler {
    ...
    private void saveCategories() {
        try {
            ...
            INbt.setInteger("LastId", nextId);
            INbt.setInteger("LastAnvilId", nextAnvilId);
            INbt.setInteger("Version", version);
            ...
            NBTIO.writeCompressed(INbt, new FileOutputStream(file));
            if (file1.exists()) {
                file1.delete();
            }
    ...
    public RecipeCarpentry saveRecipe(INbt compound) throws IOException {
        RecipeCarpentry recipe = RecipeCarpentry.create(compound);
        recipe.readNBT(compound);
    ...
    public RecipeAnvil saveAnvilRecipe(INbt compound) throws IOException {
    ...
    @Override
    public void addRecipe(String name, boolean global, IItemStack result, Object... objects) {
    ...
    @Override
    public void addRecipe(String name, boolean global, IItemStack result, int width, int height, IItemStack... objects) {
        ArrayList<IItemStack> list = new ArrayList<>();
        int var9 = objects.length;

        for (int var10 = 0; var10 < var9; ++var10) {
            IItemStack item = objects[var10];
            if (item.stackSize > 0) {
                list.add(item);
            }
        }

        RecipeCarpentry recipe = new RecipeCarpentry(width, height, list.toArray(new IItemStack[0]), result);
        recipe.isGlobal = global;
        recipe.name = name;
    ...
    @Override
    public void addAnvilRecipe(String name, boolean global, IItemStack itemToRepair, IItemStack repairMaterial, int xpCost, float repairPercentage) {
