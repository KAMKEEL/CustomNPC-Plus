...
public class PartyController implements IPartyHandler {
    ...
    public IParty createParty(IPlayer player) {
        ...
        PlayerData playerData = PlayerData.get(IPlayer);
        if (playerData.partyUUID != null)
            return getParty(playerData.partyUUID);

        Party party = new Party();
        party.addPlayer(IPlayer);
        party.setLeader(IPlayer);
        this.parties.put(party.getPartyUUID(), party);
        return party;
    }
        ...
        if (party != null) {
            ...
            for (UUID uuid : party.getPlayerUUIDs()) {
                ...
                if (playerData != null) {
                    ...
                    if (player != null) {
                        sendInviteData((IPlayer) player);
                        AchievementPacket.sendAchievement((IPlayer) player, true, "party.disbandAlert", "");
                        ChatAlertPacket.sendChatAlert((IPlayer) player, "\u00A7c", "party.disbandMessage", "!");
                    }
                }
            }
    ...
    public void sendKickMessages(Party party, IPlayer kickPlayer, String kickPlayerName) {
        if (party == null)
            return;
        ...
        for (String name : party.getPlayerNames()) {
            ...
            if (playerMP != null) {
                AchievementPacket.sendAchievement((IPlayer) playerMP, true, "party.kickOtherAlert", kickPlayerName);
                ChatAlertPacket.sendChatAlert((IPlayer) playerMP, "\u00A7e", kickPlayerName, " \u00A74", "party.kickOtherChat", "!");
            }
        }

        if (kickPlayer != null) {
            AchievementPacket.sendAchievement((IPlayer) kickPlayer, true, "party.kickYouAlert", "");
            ChatAlertPacket.sendChatAlert((IPlayer) kickPlayer, "\u00A74", "party.kickYouChat", "!");
        }
    }

    public void sendLeavingMessages(Party party, IPlayer leavingPlayer) {
        ...
        for (String name : party.getPlayerNames()) {
            IPlayer playerMP = NoppesUtilServer.getPlayerByName(name);
            if (playerMP != null) {
                AchievementPacket.sendAchievement((IPlayer) playerMP, true, "party.leaveOtherAlert", leavingPlayer.getCommandSenderName());
                ChatAlertPacket.sendChatAlert((IPlayer) playerMP, "\u00A7e", leavingPlayer.getCommandSenderName(), " \u00A7c", "party.leaveOtherChat", "!");
            }
        }
        AchievementPacket.sendAchievement((IPlayer) leavingPlayer, true, "party.leaveYouAlert", "");
        ChatAlertPacket.sendChatAlert((IPlayer) leavingPlayer, "\u00A7c", "party.leaveYouChat", "!");
    }
    ...
    public void pingPartyUpdate(Party party) {
        ...
        INbt compound = party.writeToNBT();
        ...
        for (String name : party.getPlayerNames()) {
            ...
            if (playerMP != null) {
                ...
                if (playerData != null) {
                    PartyDataPacket.sendPartyData((IPlayer) playerMP, compound);
                }
            }
        }
    }
    ...
    public void sendQuestChat(Party party, String... chatAlerts) {
        ...
        for (String name : party.getPlayerNames()) {
            ...
            if (playerMP != null) {
                ...
                ChatAlertPacket.sendChatAlert((IPlayer) playerMP, args);
            }
        }
    }
                    ...
                    if (playerMP != null) {
                        ...
                        if (playerData != null) {
                            if (playerData.questData.getTrackedQuest() != null && questData.quest.getId() == playerData.questData.getTrackedQuest().getId()) {
                                NoppesUtilPlayer.sendPartyTrackedQuestData((IPlayer) playerMP, party);
                            }
                        }
                    }
                }
