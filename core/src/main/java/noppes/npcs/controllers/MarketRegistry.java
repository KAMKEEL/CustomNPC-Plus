...
public class MarketRegistry {
    ...
    private static final Map<String, Set<IPlayer>> marketViewers = new ConcurrentHashMap<>();
     ...
     */
    public static void registerViewer(String marketName, IPlayer player) {
        if (marketName == null || marketName.isEmpty() || player == null) return;

        Set<IPlayer> viewers = marketViewers.computeIfAbsent(marketName,
            k -> Collections.newSetFromMap(new WeakHashMap<>()));
     ...
     */
    public static void unregisterViewer(String marketName, IPlayer player) {
        if (marketName == null || marketName.isEmpty() || player == null) return;

        Set<IPlayer> viewers = marketViewers.get(marketName);
        if (viewers != null) {
            synchronized (viewers) {
    ...
    public static void syncMarket(String marketName) {
        ...
        INbt marketData = Market.getMarketCache(marketName);
        if (marketData == null) return;

        // Update all traders' internal stock data
        Set<RoleTrader> traders = marketTraders.get(marketName);
        if (traders != null) {
            INbt stockData = marketData.getCompoundTag("Stock");
            ...
            synchronized (traders) {
                ...
                for (RoleTrader trader : traders) {
                    ...
                    if (stockData != null && stockData.hasKey("EnableStock")) {
                        trader.stock.readFromNBT(new NBTWrapper(stockData));
                    }
                }
            }
        }
