...
public class SyncController {
    ...
    public static void syncPlayer(IPlayer player) {
        SyncController.syncPlayerInternal(player, true);
    }

    private static void syncPlayerInternal(IPlayer player, boolean includePostPackets) {
    ...
    public static void beginLogin(IPlayer player) {
    ...
    private static void sendPostLoginPackets(IPlayer player) {
        ...
        // PlayerLoggedInEvent can arrive before the client IEntity exists.
    ...
    public static INbt workbenchNBT() {
    ...
    public static INbt carpentryNBT() {
        RecipeController controller = RecipeController.Instance;
        INbtList list = new INbtList();
        INbt compound = new INbt();
    ...
    public static INbt dialogCategoriesNBT() {
        INbt compound = new INbt();
        INbtList categoryList = new INbtList();
        for (DialogCategory category : DialogController.Instance.categories.values()) {
            INbt questCompound = new INbt();
            INbtList dialogList = new INbtList();
            for (int dialogID : category.dialogs.keySet()) {
                Dialog quest = category.dialogs.get(dialogID);
                dialogList.appendTag(quest.writeToNBT(new INbt()));
            }
            questCompound.setTag("Data", dialogList);
            questCompound.setTag("CatNBT", category.writeSmallNBT(new INbt()));
    ...
    public static INbt questCategoriesNBT() {
        INbt compound = new INbt();
        INbtList categoryList = new INbtList();
        for (QuestCategory category : QuestController.Instance.categories.values()) {
            ...
            for (int questID : category.quests.keySet()) {
                ...
                questList.appendTag(quest.writeToNBT(new INbt()));
            }
    ...
    public static INbt customEffectsNBT() {
        INbtList list = new INbtList();
        INbt compound = new INbt();
    ...
    public static void syncPlayerData(IPlayer player, boolean update) {
        PlayerData data = PlayerData.get(player);
        if (data != null) {
            if (update) {
    ...
    @ClientOnly
    public static void clientSync(EnumSyncType enumSyncType, int revision, INbt fullCompound) {
        switch (enumSyncType) {
            case FACTION: {
                INbtList list = fullCompound.getTagList("Factions", 10);
            ...
            case DIALOG_CATEGORY: {
                if (!fullCompound.hasNoTags()) {
                    INbtList categories = fullCompound.getTagList("DialogCategories", 10);
                    for (int j = 0; j < fullCompound.getTagList("DialogCategories", 10).tagCount(); j++) {
                        INbt categoryCompound = categories.getCompoundTagAt(j);
                        if (categoryCompound.hasNoTags())
                            continue;

                        DialogCategory category = new DialogCategory();
                        category.readSmallNBT(categoryCompound.getCompoundTag("CatNBT"));
                        INbtList dialogList = categoryCompound.getTagList("Data", 10);
            ...
            case QUEST_CATEGORY: {
                if (!fullCompound.hasNoTags()) {
                    INbtList categories = fullCompound.getTagList("QuestCategories", 10);
                    for (int j = 0; j < fullCompound.getTagList("QuestCategories", 10).tagCount(); j++) {
                        ...
                        category.readSmallNBT(categoryCompound.getCompoundTag("CatNBT"));
                        INbtList questList = categoryCompound.getTagList("Data", 10);
            ...
            case MAGIC: {
                INbtList list = fullCompound.getTagList("Data", 10);
            ...
            case WORKBENCH_RECIPES: {
                INbtList list = fullCompound.getTagList("recipes", 10);
                if (list == null)
                    return;

                for (int i = 0; i < list.tagCount(); i++) {
                    INbt recipeCompound = list.getCompoundTagAt(i);
            ...
            case CUSTOM_ABILITY: {
                ...
                for (int i = 0; i < list.tagCount(); i++) {
                    INbt nbt = list.getCompoundTagAt(i);
     ...
     */
    @ClientOnly
    public static void clientUpdate(EnumSyncType enumSyncType, int category_id, int revision, INbt compound) {
        switch (enumSyncType) {
            case FACTION: {
                Faction faction = new Faction();
            ...
            case QUEST_CATEGORY: {
                ...
                INbtList list = compound.getTagList("Data", 10);
                if (QuestController.Instance.categoriesSync.containsKey(category.id)) {
                    category = QuestController.Instance.categoriesSync.get(category.id);
                    category.readSmallNBT(compound.getCompoundTag("CatNBT"));
                }
    ...
    public static void syncUpdate(EnumSyncType type, int cat, INbt compound) {
    ...
    public static INbt updateQuestCat(QuestCategory questCategory) {
        INbt questCompound = new INbt();
        INbtList questList = new INbtList();
        ...
        questCompound.setTag("Data", questList);
        questCompound.setTag("CatNBT", questCategory.writeSmallNBT(new INbt()));
        return questCompound;
    }

    public static INbt updateDialogCat(DialogCategory dialogCategory) {
        INbt dialogCompound = new INbt();
        INbtList dialogList = new INbtList();
        for (int questID : dialogCategory.dialogs.keySet()) {
            Dialog dialog = dialogCategory.dialogs.get(questID);
            dialogList.appendTag(dialog.writeToNBT(new INbt()));
        }
        dialogCompound.setTag("Data", dialogList);
        dialogCompound.setTag("CatNBT", dialogCategory.writeSmallNBT(new INbt()));
    ...
    public static void syncEffects(IPlayer playerMP) {
        ...
        INbt compound = playerData.getPlayerEffects();
     ...
     * Used for IEntity reconstruction and other full-refresh scenarios.
     */
    public static void syncAbilities(IPlayer playerMP) {
        PlayerData playerData = PlayerData.get(playerMP);
     ...
     */
    public static void syncAbilityCooldowns(IPlayer playerMP) {
        PlayerData playerData = PlayerData.get(playerMP);
        if (playerData == null) return;
    ...
    @ClientOnly
    public static void clientSyncEffects(INbt compound) {
    ...
    private static void registerCache(EnumSyncType type, Supplier<INbt> supplier) {
    ...
    private static final class SyncCacheEntry {
        private final Supplier<INbt> supplier;
        private volatile CachedSyncPayload payload;
        private int revision = 0;
        private boolean dirty = true;

        private SyncCacheEntry(EnumSyncType type, Supplier<INbt> supplier) {
