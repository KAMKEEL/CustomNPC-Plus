...
public class ProfileController implements IProfileHandler {
    ...
    public synchronized void login(IPlayer player) {
    ...
    public synchronized INbt load(IPlayer player) {
    ...
    public synchronized INbt load(UUID uuid) {
        File saveDir = getProfileDir();
    ...
    public synchronized void saveOffline(Profile profile, UUID uuid) {
        ...
        final INbt compound = profile.writeToNBT();
        ...
        CustomNPCsThreader.customNPCThread.execute(() -> {
            try {
                ...
                NBTIO.writeCompressed(compound, new FileOutputStream(fileNew));
                if (fileOld.exists()) {
                    fileOld.delete();
                }
    ...
    public synchronized void save(IPlayer player, Profile profile) {
        profile.setLocked(true);
        final INbt compound = profile.writeToNBT();
    ...
    private void backupProfile(UUID uuid, INbt compound) {
        try {
            ...
            NBTIO.writeCompressed(compound, new FileOutputStream(backupFile));
    ...
    public boolean rollbackProfile(String username, File backupFile) {
        ...
        try (FileInputStream fis = new FileInputStream(backupFile)) {
            INbt compound = NBTIO.readCompressed(fis);
            File saveDir = getProfileDir();
            ...
            IPlayer player = NoppesUtilServer.getPlayerByName(username);
            Profile newProfile;
            if (player != null) {
    ...
    public synchronized void logout(IPlayer player) {
    ...
    public Profile getProfile(IPlayer player) {
        if (player == null)
            return null;
        if (!activeProfiles.containsKey(player.getUniqueID()))
            login(player);
    ...
    private ProfileOperation cloneSlotInternal(Profile profile, int sourceSlotId, int destinationSlotId, boolean temporary) {
        ...
        Map<String, INbt> newComponents = new HashMap<>();
        for (String key : sourceSlot.getComponents().keySet()) {
            newComponents.put(key, (INbt) sourceSlot.getComponentData(key).copy());
        }
    ...
    public ProfileOperation createSlotInternal(Profile profile) {
        ...
        if (profile.player != null) {
            ...
            if (profile.player instanceof IPlayer) {
                ProfileSharedQuestPacket.sendToPlayer((IPlayer) profile.player);
            }
    ...
    public ProfileOperation cloneSlot(IPlayer player, int sourceSlotId, int destinationSlotId, boolean temporary) {
    ...
    public ProfileOperation removeSlot(IPlayer player, int slotId) {
        Profile profile = getProfile(player);
        if (profile == null)
            return ProfileOperation.error(MSG_PLAYER_NOT_FOUND);
    ...
    public ProfileOperation changeSlot(IPlayer player, int newSlotId) {
        Profile profile = getProfile(player);
        if (profile == null)
            return ProfileOperation.error(MSG_PLAYER_NOT_FOUND);
    ...
    private void saveSlotData(IPlayer player, Profile profile, int slotId) {
        ...
        for (IProfileData profileData : profileTypes.values()) {
            INbt cloned = (INbt) profileData.getCurrentNBT(player).copy();
    ...
    private void loadSlotData(IPlayer player, Profile profile, int slotId) {
        if (player == null || profile == null)
            return;
        ...
        for (IProfileData profileData : profileTypes.values()) {
            ...
            if (slot.getComponents().containsKey(profileData.getTagName()))
                data = (INbt) slot.getComponentData(profileData.getTagName()).copy();
            else
                data = new INbt();
    ...
    public void loadSlotData(IPlayer player) {
        if (player == null || !activeProfiles.containsKey(player.getUniqueID()))
            return;
        Profile profile = activeProfiles.get(player.getUniqueID());
    ...
    public List<ProfileInfoEntry> getProfileInfo(IPlayer player, int slotId) {
        ...
        if (slotId == profile.getCurrentSlotId()) {
            for (IProfileData pd : dataList) {
                INbt currentNBT = pd.getCurrentNBT(player);
            ...
            for (IProfileData pd : dataList) {
                if (slot.getComponents().containsKey(pd.getTagName())) {
                    INbt sub = (INbt) slot.getComponentData(pd.getTagName()).copy();
    ...
    public boolean allowSlotPermission(IPlayer player) {
        Profile profile = getProfile(player);
    ...
    public PlayerData getSlotPlayerData(IPlayer player, Slot slot) {
        if (slot == null)
            return null;
        ...
        INbt compound = slot.getComponentData(new CNPCData().getTagName());
        if (compound == null)
            compound = new INbt();
        else
            compound = (INbt) compound.copy();
        playerData.setNBT(compound);
        return playerData;
    }

    public void verifySlotQuests(IPlayer player) {
        Profile profile = getProfile(player);
        if (profile == null) {
            return;
        }
    ...
    public void syncSharedQuestsToPlayer(IPlayer player) {
        Profile profile = getProfile(player);
    ...
    public void shareQuestCompletion(IPlayer player, int questId, long completeTime) {
        Profile profile = getProfile(player);
        if (profile == null) {
            return;
        }
        ...
        if (player instanceof IPlayer) {
            ProfileSharedQuestPacket.sendToPlayer((IPlayer) player);
        }
    }
}
