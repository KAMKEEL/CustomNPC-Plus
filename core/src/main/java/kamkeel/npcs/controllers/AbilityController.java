...
public class AbilityController implements IAbilityHandler {
    ...
    private final List<Predicate<IPlayer>> flightCheckers = new ArrayList<>();
    private final List<Predicate<IPlayer>> activationCheckers = new ArrayList<>();
    ...
    public Ability fromNBT(INbt nbt) {
    ...
    private void loadAbilitiesFromDir(File dir, int catId) {
        ...
        for (File file : files) {
            ...
            try {
                ...
                INbt nbt = NBTJsonUtil.LoadFile(file);
    ...
    public boolean saveCustomAbility(Ability ability) {
        ...
        try {
            INbt nbt = ability.writeNBT(true);
    ...
    public Ability cloneCustomAbility(String originalName) {
        ...
        INbt nbt = original.writeNBT(true);
    ...
    public void registerFlightChecker(Predicate<IPlayer> checker) {
        flightCheckers.add(checker);
    }

    public boolean isPlayerFlying(IPlayer player) {
        if (player.capabilities.isFlying) return true;
        for (Predicate<IPlayer> checker : flightCheckers) {
            if (checker.test(player)) return true;
        }
        return false;
    }

    public void registerActivationChecker(Predicate<IPlayer> checker) {
        activationCheckers.add(checker);
    }

    public boolean canPlayerActivate(IPlayer player) {
        for (Predicate<IPlayer> checker : activationCheckers) {
     ...
     */
    public boolean fireOnAbilityStart(Ability ability, IEntityLivingBase caster, IEntityLivingBase target) {
     ...
     */
    public boolean fireOnAbilityTick(Ability ability, IEntityLivingBase caster, IEntityLivingBase target,
     ...
     */
    public void fireOnAbilityComplete(Ability ability, IEntityLivingBase caster, IEntityLivingBase target,
                                      boolean interrupted) {
        for (IAbilityExtender ext : extenders) {
     ...
     */
    public boolean fireOnAbilityDamage(Ability ability, IEntityLivingBase caster, IEntityLivingBase target,
     ...
     */
    public float fireModifyProjectileDamage(Ability ability, IEntityLivingBase caster, float baseDamage) {
     ...
     */
    public float fireModifyBarrierHealth(Ability ability, IEntityLivingBase caster, float baseHealth) {
     ...
     */
    public boolean fireOnAbilityHeal(Ability ability, IEntityLivingBase caster, IEntityLivingBase target, float healAmount) {
        for (IAbilityExtender ext : extenders) {
    ...
    public Boolean fireCheckConditions(AbilityCondition condition, IEntityLivingBase caster, IEntityLivingBase target) {
        for (IAbilityExtender ext : extenders) {
    ...
    public Boolean fireCheckConditionsForPlayer(AbilityCondition condition, IEntityLivingBase player) {
        for (IAbilityExtender ext : extenders) {
