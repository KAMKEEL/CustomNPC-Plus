 ...
 * <p>
 * Subclasses provide IEntity creation/firing/charging logic and type-specific fields.
 *
 * @param <E> The IEntity type this ability spawns
 */
public abstract class AbilityEnergyProjectile<E extends EntityEnergyProjectile> extends AbilityEnergy implements IAbilityEnergyProjectile {
    ...
    protected transient IEntityLivingBase chargeVisualCaster;
    ...
    /**
     * Create a single IEntity for the given projectile index.
     * Called when a projectile actually fires.
     */
    protected abstract E createEntity(IEntityLivingBase caster, IEntityLivingBase target,
                                      IVector3 spawnPos, EnergyDisplayData resolved, int index);

    /**
     * Fire an IEntity (start moving toward target). IEntity is guaranteed non-null and alive.
     */
    protected abstract void fireEntity(E IEntity, IEntityLivingBase target);

    /**
     * Setup IEntity for charging animation during windup (non-preview).
     */
    protected abstract void setupEntityCharging(E IEntity, ProjectileData projData, int index);

    /**
     * Setup IEntity for preview mode during windup.
     */
    protected abstract void setupEntityPreview(E IEntity, IEntityLivingBase caster,
     ...
     */
    protected abstract void writeTypeSpecificNBT(INbt nbt);

    /**
     * Read type-specific fields from NBT.
     * Shared data classes are read by the base class.
     */
    protected abstract void readTypeSpecificNBT(INbt nbt);
     ...
     */
    protected IVector3 getSpawnPosition(IEntityLivingBase caster, int index) {
    ...
    protected void initRuntimeState(IEntityLivingBase caster) {
    ...
    protected E createProjectileEntity(IEntityLivingBase caster, IEntityLivingBase target, int index) {
        IVector3 spawnPos = getSpawnPosition(caster, index);
        EnergyDisplayData resolved = projectiles[index].resolveDisplay(displayData);
        E IEntity = createEntity(caster, target, spawnPos, resolved, index);
        IEntity.setAnchorData(projectiles[index].anchor);
        IEntity.setEffects(this.effects);
        IEntity.setIgnoreIFrames(this.isIgnoreIFrames());
        IEntity.setSourceAbility(this);
        MagicData resolvedMagic = resolveMagicData(caster);
        if (resolvedMagic != null) {
            IEntity.setMagicData(resolvedMagic.copy());
        }
        return IEntity;
    }

    protected void spawnProjectileEntity(E IEntity, int index) {
        if (IEntity == null) return;
        spawnAbilityEntity(IEntity);
        entities[index] = IEntity;
    ...
    protected void spawnAndFireProjectile(IEntityLivingBase caster, IEntityLivingBase target, int index) {
        if (entities != null && entities[index] != null && !entities[index].isDead) return;
        E IEntity = createProjectileEntity(caster, target, index);
        ...
        fireEntitySafe(IEntity, caster, target);
        spawnProjectileEntity(IEntity, index);
    }
     ...
     */
    protected IEntityLivingBase getLaunchTarget(IEntityLivingBase caster, IEntityLivingBase target) {
        return isPlayerCaster(caster) ? null : target;
    }

    /**
     * Fire an IEntity if it's valid.
     * Clears charging state first so the IEntity can move and check collisions.
     */
    protected void fireEntitySafe(E IEntity, IEntityLivingBase caster, IEntityLivingBase target) {
        if (IEntity != null && !IEntity.isDead) {
            if (IEntity.isCharging()) {
                IEntity.setCharging(false);
            }
            fireEntity(IEntity, getLaunchTarget(caster, target));
        }
    }

    protected String getChargeVisualId(IEntityLivingBase caster, int index) {
        return "energy_charge:" + caster.getEntityId() + ":" + executionStartTime + ":" + burstIndex + ":" + index;
    }

    protected void spawnChargeVisual(IEntityLivingBase caster, IEntityLivingBase target, int index, int chargeDuration) {
    ...
    protected void removeChargeVisual(IEntityLivingBase caster, int index) {
        if (isPreview() || caster == null || caster.worldObj == null || caster.worldObj.isRemote) return;
        if (chargeVisualIds == null || index < 0 || index >= chargeVisualIds.length) return;
    ...
    protected void removeAllChargeVisuals(IEntityLivingBase caster) {
    ...
    /**
     * Remove a specific IEntity from this ability's tracking.
    ...
    public void detachEntity(IEntity IEntity) {
        if (entities == null || IEntity == null) return;
        for (int i = 0; i < entities.length; i++) {
            if (entities[i] == IEntity) {
                entities[i] = null;
                break;
            }
        }
    ...
    public void onExecute(IEntityLivingBase caster, IEntityLivingBase target) {
        // GUI preview keeps the original non-IWorld preview IEntity flow.
        if (isPreview()) {
            if (entities == null) {
                initRuntimeState(caster);
                for (int i = 0; i < projectileCount; i++) {
    ...
    public void onActiveTick(IEntityLivingBase caster, IEntityLivingBase target, int tick) {
        ...
        // Fire staggered projectiles: spawn real IWorld IEntity only when it is time to fire.
        if (fireDelay > 0) {
            for (int i = 1; i < projectileCount; i++) {
                if (tick == fireDelay * i) {
                    removeChargeVisual(caster, i);
        ...
        if (allDead) {
            for (E IEntity : entities) {
                if (IEntity == null || IEntity.isDead) continue;

                if (!isPreview() && tick > 5 && IEntity.worldObj != null
                    && IEntity.worldObj.getEntityByID(IEntity.getEntityId()) != IEntity) {
                    IEntity.setDead();
                    continue;
                }
