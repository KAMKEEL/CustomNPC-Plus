...
public class AbilitySlam extends Ability implements IAbilitySlam {
    ...
    private transient boolean hasRisen = false; // Whether IEntity has risen above launch position
     ...
     */
    private void executeNpcSlam(IEntityLivingBase caster, IEntityLivingBase target) {
        if (targetingMode == TargetingMode.AOE_SELF) {
            targetX = caster.posX;
            targetY = caster.posY;
            targetZ = caster.posZ;
    ...
    /**
     * Player slam: behavior depends on whether the player is on the ground or in the air.
     * <p>
     * On ground: Launch in look direction using ballistic arc, full damage on landing.
     * In air: Slam straight down from current height, damage scales with fall distance.
     * Works regardless of flying state — position is forced downward each tick.
     */
    private void executePlayerSlam(IEntityLivingBase caster) {
     ...
     */
    private void launchTowardTarget(IEntityLivingBase caster) {
        ...
        if (!isPreview()) {
            // Use direct S12 packet for players to ensure velocity reaches client
            // before flight code can override it
            if (caster instanceof IPlayer) {
                ((IPlayer) caster).playerNetServerHandler.sendPacket(
                    new S12PacketEntityVelocity(caster));
            } else {
    ...
    public void onActiveTick(IEntityLivingBase caster, IEntityLivingBase target, int tick) {
        ...
        // Track if IEntity has risen above launch position.
        ...
        if (airTicks > graceTicks) {
            ...
            // Safety: if IEntity hasn't risen after triple the grace period,
     ...
     */
    private void onLanding(IEntityLivingBase caster, IWorld IWorld) {
        hasLanded = true;
        if (!isPreview() && caster instanceof EntityNPCInterface) {
        ...
        IWorld.playSoundAtEntity(caster, "random.explode", 1.0f, 1.0f);
        ...
        List<IEntity> entities = IWorld.getEntitiesWithinAABBExcludingEntity(
        ...
        for (IEntity IEntity : entities) {
            if (IEntity instanceof IEntityLivingBase && IEntity != caster) {
                ...
                if (dx * dx + dz * dz <= radius * radius) {
                    ...
                    boolean wasHit = applyAbilityDamage(caster, livingTarget, effectiveDamage, knockbackStrength);
                    if (wasHit) {
        ...
        // Spawn particles
        spawnSlamParticles(IWorld, caster.posX, caster.posY, caster.posZ);
     ...
     */
    private void spawnSlamParticles(IWorld IWorld, double x, double y, double z) {
        // Spawn IExplosion particles in a ring
        for (int i = 0; i < 20; i++) {
            double angle = (i / 20.0) * Math.PI * 2;
            double px = x + Math.cos(angle) * radius * 0.8;
            double pz = z + Math.sin(angle) * radius * 0.8;
            IWorld.spawnParticle("explode", px, y + 0.5, pz, 0, 0.1, 0);
        }

        // Spawn smoke at center
        for (int i = 0; i < 10; i++) {
            IWorld.spawnParticle("smoke", x, y + 0.2, z,
                (IWorld.rand.nextDouble() - 0.5) * 0.3,
                IWorld.rand.nextDouble() * 0.2,
                (IWorld.rand.nextDouble() - 0.5) * 0.3);
        }

        // Ground crack particles - check if block is valid
        net.minecraft.block.Block groundBlock = IWorld.getBlock((int) x, (int) y - 1, (int) z);
        if (groundBlock != null && groundBlock.getMaterial().isSolid()) {
            int blockId = net.minecraft.block.Block.getIdFromBlock(groundBlock);
            if (blockId > 0) {
                for (int i = 0; i < 15; i++) {
                    double angle = IWorld.rand.nextDouble() * Math.PI * 2;
                    double dist = IWorld.rand.nextDouble() * radius * 0.6;
                    double px = x + Math.cos(angle) * dist;
                    double pz = z + Math.sin(angle) * dist;
                    IWorld.spawnParticle("blockcrack_" + blockId + "_0", px, y + 0.1, pz, 0, 0.2, 0);
                }
            }
        }
    }
     ...
     */
    private void restoreFlightIfNeeded(IEntityLivingBase caster) {
        if (airSlam && wasFlying && caster instanceof IPlayer) {
            IPlayer mp = (IPlayer) caster;
