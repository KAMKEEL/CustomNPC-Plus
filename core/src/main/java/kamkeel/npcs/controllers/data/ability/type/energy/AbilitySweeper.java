 ...
 * <p>
 * All damage logic is handled by the EntityEnergySweeper IEntity to ensure
...
public class AbilitySweeper extends AbilityEnergy implements IAbilitySweeper {
    ...
    public void onExecute(IEntityLivingBase caster, IEntityLivingBase target) {
        // Spawn the sweeper IEntity that handles BOTH visuals AND damage.
    ...
    public void onActiveTick(IEntityLivingBase caster, IEntityLivingBase target, int tick) {
        // Signal completion when IEntity dies
    ...
    @Override
    public void cleanup() {
        // Clean up IEntity if still alive
        if (activeEntity != null && !activeEntity.isDead) {
