...
public class AbilityCharge extends AbilityMovement implements IAbilityCharge {
    ...
    public void onActiveTick(IEntityLivingBase caster, IEntityLivingBase target, int tick) {
        ...
        if (!caster.worldObj.isRemote && !isPreview()) {
            IBoundingBox hitBox = caster.boundingBox.expand(hitWidth, hitWidth * 0.5, hitWidth);

            @SuppressWarnings("unchecked")
            List<IEntity> entities = caster.worldObj.getEntitiesWithinAABB(IEntityLivingBase.class, hitBox);
            ...
            for (IEntity IEntity : entities) {
                ...
                IEntityLivingBase livingEntity = (IEntityLivingBase) IEntity;
    ...
    @Override
    public void onInterrupt(IEntityLivingBase caster, noppes.npcs.api.IDamageSource source, float damage) {
        stopMomentum(caster);
