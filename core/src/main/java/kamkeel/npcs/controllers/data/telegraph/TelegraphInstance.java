...
public class TelegraphInstance {
    ...
    private int targetEntityId = -1;  // For LINE telegraphs: IEntity to face (yaw tracking)
    private boolean trackFollowedEntityYaw = false; // Track followed IEntity's rotationYaw when no targetEntity
    ...
    private transient IWorld IWorld;
     ...
     *
     * @param IWorld The IWorld (for IEntity lookup)
    ...
    public boolean tick(IWorld IWorld) {
        ...
        // Update position if following an IEntity
        if (entityIdToFollow >= 0 && IWorld != null) {
            IEntity IEntity = IWorld.getEntityByID(entityIdToFollow);
            if (IEntity != null) {
                this.x = IEntity.posX;
                this.z = IEntity.posZ;
                // Find ground level below the IEntity for proper telegraph placement
                int searchRange = (telegraph != null) ? telegraph.getGroundSearchRange() : 3;
                this.y = Ability.findGroundLevel(IWorld, IEntity.posX, IEntity.posY, IEntity.posZ, searchRange);

                // Track followed IEntity's yaw for directional telegraphs (player casters)
                if (trackFollowedEntityYaw && targetEntityId < 0) {
                    this.yaw = IEntity.rotationYaw;
                }
            }
        }

        // Update yaw to face target IEntity (for LINE telegraphs)
        if (targetEntityId >= 0 && IWorld != null) {
            IEntity target = IWorld.getEntityByID(targetEntityId);
            if (target != null) {
    ...
    public INbt writeNBT() {
        ...
        if (telegraph != null) {
            nbt.setTag("telegraph", ((NBTWrapper) telegraph.writeNBT()).getMCTag());
        }
    ...
    public void readNBT(INbt nbt) {
        ...
        if (nbt.hasKey("telegraph")) {
            ...
            this.telegraph.readNBT(new NBTWrapper(nbt.getCompoundTag("telegraph")));
        }
     ...
     * Stops following any IEntity - used when ability commits to action.
     */
    ...
    public void setWorld(IWorld IWorld) {
        this.IWorld = IWorld;
        if (IWorld != null) {
            this.dimensionId = IWorld.provider.dimensionId;
        }
    }
