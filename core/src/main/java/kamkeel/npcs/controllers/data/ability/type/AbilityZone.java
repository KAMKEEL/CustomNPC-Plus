...
public abstract class AbilityZone extends Ability {
    ...
    public void onActiveTick(IEntityLivingBase caster, IEntityLivingBase target, int tick) {
        ...
        while (it.hasNext()) {
            EntityAbilityZone IEntity = it.next();
    ...
    @Override
    public void cleanup() {
        for (EntityAbilityZone IEntity : activeEntities) {
            if (IEntity != null && !IEntity.isDead) {
                killAbilityEntity(IEntity);
            }
        }
        activeEntities.clear();
        preCalculatedPositions.clear();
    ...
    protected double[] findSpawnPosition(IEntityLivingBase caster, List<double[]> placedPositions, float minSeparation) {
    ...
    protected void applyVisualToEntity(EntityAbilityZone IEntity) {
        IEntity.applyVisual(groundFill, groundAlpha,
    ...
    protected void writeZoneNBT(INbt nbt) {
        ...
        colorData.writeNBT(new NBTWrapper(nbt));
    ...
    protected void readZoneNBT(INbt nbt, int defaultDuration) {
        ...
        colorData.readNBT(new NBTWrapper(nbt));
