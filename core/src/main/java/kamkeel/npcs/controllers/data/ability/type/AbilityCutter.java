...
public class AbilityCutter extends Ability implements IAbilityCutter {
    ...
    private void playActiveSound(IEntityLivingBase caster, IWorld IWorld) {
        activeSoundPlayed = true;
        if (activeSound != null && !activeSound.isEmpty()) {
            IWorld.playSoundAtEntity(caster, activeSound, 1.0f, 1.0f);
        }
    }
     ...
     */
    private void performSweepDamageRange(IEntityLivingBase caster, IWorld IWorld, float minDist, float maxDist,
        ...
        for (IEntityLivingBase IEntity : entities) {
            ...
            if (!hasLineOfSight(IWorld, caster, IEntity)) continue;
            if (isBlockedByBarrier(IWorld, caster, IEntity)) continue;

            hitEntities.add(IEntity.getEntityId());
            ...
            boolean wasHit = applyAbilityDamage(caster, IEntity, actualDamage, knockback);
    ...
    /**
     * Check if an IEntity falls within the angular range between two sweep positions.
