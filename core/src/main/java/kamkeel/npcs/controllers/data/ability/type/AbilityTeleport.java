...
public class AbilityTeleport extends Ability implements IAbilityTeleport {
    ...
    private void performBlink(IEntityLivingBase caster, IEntityLivingBase target, IWorld IWorld) {
        if (IWorld.isRemote && !isPreview()) return;

        double oldX = caster.posX;
        double oldY = caster.posY;
        double oldZ = caster.posZ;

        IVector3 destination = calculateDestination(caster, target, IWorld);
        if (destination == null) return;

        if (!isPreview()) {
            boolean mustHaveLOS = mode == TeleportMode.BEHIND || requireLineOfSight;
            if (mustHaveLOS && !hasLineOfSight(IWorld, oldX, oldY + caster.getEyeHeight(), oldZ,
                destination.xCoord, destination.yCoord + caster.getEyeHeight(), destination.zCoord)) {
                destination = findValidPositionAlongLine(IWorld, caster, oldX, oldY, oldZ,
                    destination.xCoord, destination.yCoord, destination.zCoord);
                if (destination == null) return;
            }

            destination = findSafeDestination(IWorld, oldX, oldY, oldZ,
                destination.xCoord, destination.yCoord, destination.zCoord);
            if (destination == null) return;

            // Damage at origin
            if (damageAtStart) {
                dealDamageAt(caster, IWorld, oldX, oldY, oldZ);
            }

            // Spawn particles at origin
            spawnTeleportParticles(IWorld, oldX, oldY, oldZ);
        }
        ...
        if (!isPreview()) {
            ...
            spawnTeleportParticles(IWorld, destination.xCoord, destination.yCoord, destination.zCoord);

            // Damage at destination
            if (damageAtEnd) {
                dealDamageAt(caster, IWorld, destination.xCoord, destination.yCoord, destination.zCoord);
            }
        }
    ...
    private void spawnTeleportParticles(IWorld IWorld, double x, double y, double z) {
        for (int i = 0; i < 20; i++) {
            IWorld.spawnParticle("portal",
    ...
    private IVector3 calculateDestination(IEntityLivingBase caster, IEntityLivingBase target, IWorld IWorld) {
        double newX;
        double newY;
        double newZ;
        ...
        if (target != null) {
            switch (mode) {
                case BEHIND:
                    ...
                    newY = target.posY;
                    return IVector3.createVectorHelper(newX, newY, newZ);
                case SINGLE:
                case BLINK:
                default:
    ...
    private IVector3 findSafeDestination(IWorld IWorld, double oldX, double oldY, double oldZ,
        ...
        double safeY = findSafeYNear(IWorld, destX, destY, destZ);
        IVector3 safe = IVector3.createVectorHelper(destX, safeY, destZ);

        if (!isSafeLocation(IWorld, (int) Math.floor(safe.xCoord),
    ...
    private double findSafeYNear(IWorld IWorld, double x, double baseY, double z) {
        int[] offsets = new int[]{0, 1, -1, 2, -2};
        for (int offset : offsets) {
            double y = baseY + offset;
            if (isSafeLocation(IWorld, (int) Math.floor(x), (int) Math.floor(y), (int) Math.floor(z))) {
                return y;
            }
        }
        return findSafeY(IWorld, x, baseY, z);
    }

    private boolean hasLineOfSight(IWorld IWorld, double x1, double y1, double z1,
        ...
        for (double d = step; d < distance; d += step) {
            ...
            int blockX = ValueUtil.floorDouble(checkX);
            int blockY = ValueUtil.floorDouble(checkY);
            int blockZ = ValueUtil.floorDouble(checkZ);

            // Skip out-of-IWorld coordinates
            if (blockY < 0 || blockY >= 256) {
                continue;
            }

            Block block = IWorld.getBlock(blockX, blockY, blockZ);
    ...
    private IVector3 findValidPositionAlongLine(IWorld IWorld, IEntityLivingBase caster,
        ...
        for (double d = 1.0; d < distance; d += step) {
            ...
            if (hasLineOfSight(IWorld, x1, y1 + caster.getEyeHeight(), z1,
                checkX, checkY + caster.getEyeHeight(), checkZ)) {
                int blockX = ValueUtil.floorDouble(checkX);
                int blockY = ValueUtil.floorDouble(checkY);
                int blockZ = ValueUtil.floorDouble(checkZ);

                if (isSafeLocation(IWorld, blockX, blockY, blockZ)) {
                    lastValid = IVector3.createVectorHelper(checkX, checkY, checkZ);
                }
            } else {
                break;
            }
    ...
    private double findSafeY(IWorld IWorld, double x, double baseY, double z) {
        int blockX = ValueUtil.floorDouble(x);
        int blockZ = ValueUtil.floorDouble(z);
        int blockY = ValueUtil.floorDouble(baseY);

        for (int offset = 0; offset <= 5; offset++) {
            if (isSafeLocation(IWorld, blockX, blockY + offset, blockZ)) {
                return blockY + offset;
            }
            if (offset > 0 && isSafeLocation(IWorld, blockX, blockY - offset, blockZ)) {
                return blockY - offset;
            }
        }
    ...
    private boolean isSafeLocation(IWorld IWorld, int x, int y, int z) {
        // Bounds check - ensure we're within valid IWorld coordinates
        if (y < 1 || y >= 255) {
            return false;
        }

        Block groundBlock = IWorld.getBlock(x, y - 1, z);
        Block feetBlock = IWorld.getBlock(x, y, z);
        Block headBlock = IWorld.getBlock(x, y + 1, z);
    ...
    private void dealDamageAt(IEntityLivingBase caster, IWorld IWorld, double x, double y, double z) {
