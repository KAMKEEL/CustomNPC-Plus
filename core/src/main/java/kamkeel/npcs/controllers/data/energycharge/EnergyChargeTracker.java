 ...
 * <p>
 * Keyed by caster IEntity ID so {@code PlayerEvent.StartTracking} can look up
 * active charges for a newly tracked IEntity.
 */
public class EnergyChargeTracker {
     ...
     */
    public void sendToPlayer(int casterEntityId, IPlayer player, int currentWorldTick) {
        ...
        for (ChargeEntry entry : entries) {
            ...
            INbt adjustedNbt = (INbt) entry.spawnNbt.copy();
            if (elapsed > 0) {
