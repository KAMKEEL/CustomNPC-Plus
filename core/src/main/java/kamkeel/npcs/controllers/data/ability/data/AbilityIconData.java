...
public class AbilityIconData {
    ...
    public static class Layer {
        ...
        public void writeToNBT(INbt nbt) {
    ...
    private AbilityIconData(INbt customData) {
     ...
     */
    public static AbilityIconData fromCustomData(INbt customData) {
    ...
    public void writeToNBT(INbt nbt) {
        ...
        INbtList layerList = new INbtList();
        for (int i = 0; i < MAX_LAYERS; i++) {
            INbt layerNBT = new INbt();
    ...
    public void readFromNBT(INbt nbt) {
        ...
        if (nbt.hasKey("Layers")) {
            INbtList layerList = nbt.getTagList("Layers", 10);
            for (int i = 0; i < MAX_LAYERS; i++) {
                layers[i] = new Layer();
        ...
        if (nbt.hasKey("StateIcons")) {
            INbtList list = nbt.getTagList("StateIcons", 10);
            stateIcons = new int[list.tagCount()][2];
            for (int i = 0; i < list.tagCount(); i++) {
                INbt comp = list.getCompoundTagAt(i);
    ...
    // ═══════════════════════════════════════════════════════════════════
    // SHARED IConfigProperty GETTERS/SETTERS (with auto-save)
    // ═══════════════════════════════════════════════════════════════════
