...
public class AbilityProjectile extends Ability implements IAbilityProjectile {
     ...
     */
    private void executeNpcProjectile(IEntityLivingBase caster, IEntityLivingBase target, IWorld IWorld) {
        if (isPreview()) return;
        ...
        IWorld.playSoundAtEntity(caster, "random.bow", 1.0f, 0.8f);

        if (explosive && explosionRadius > 0) {
            applyExplosionDamage(caster, target, IWorld, target.posX, target.posY, target.posZ);
        }

        spawnProjectileParticles(IWorld, caster, target);
    }

    /**
     * Player projectile: fires instant hit-scan in look direction.
     * Raycasts for the first IEntity in range and applies damage.
     */
    private void executePlayerProjectile(IEntityLivingBase caster, IWorld IWorld) {
        if (isPreview()) return;

        IVector3 look = caster.getLookVec();
        double eyeX = caster.posX;
        double eyeY = caster.posY + caster.getEyeHeight();
        double eyeZ = caster.posZ;

        // Raycast: find the closest IEntity along look direction
        IEntityLivingBase hitTarget = null;
        double closestDist = maxRange;

        @SuppressWarnings("unchecked")
        List<IEntity> candidates = IWorld.getEntitiesWithinAABBExcludingEntity(caster,
            caster.boundingBox.expand(maxRange, maxRange, maxRange));

        for (IEntity IEntity : candidates) {
            if (!(IEntity instanceof IEntityLivingBase)) continue;
            if (IEntity.isDead) continue;

            // Check if IEntity is roughly along look direction
            double dx = IEntity.posX - eyeX;
            double dy = (IEntity.posY + IEntity.height / 2) - eyeY;
            double dz = IEntity.posZ - eyeZ;
            ...
            if (dist < closestDist) {
                closestDist = dist;
                hitTarget = (IEntityLivingBase) IEntity;
            }
        }
        ...
        if (hitTarget != null) {
            ...
            if (explosive && explosionRadius > 0) {
                applyExplosionDamage(caster, hitTarget, IWorld, hitTarget.posX, hitTarget.posY, hitTarget.posZ);
            }

            spawnProjectileParticles(IWorld, caster, hitTarget);
        } else {
            // No hit — spawn particles along look direction anyway
            spawnProjectileParticlesInDirection(IWorld, caster, look);
        }
    }

    /**
     * Apply IExplosion splash damage around a point.
     */
    private void applyExplosionDamage(IEntityLivingBase caster, IEntityLivingBase primaryTarget, IWorld IWorld,
                                      double x, double y, double z) {
        @SuppressWarnings("unchecked")
        List<IEntity> entities = IWorld.getEntitiesWithinAABBExcludingEntity(primaryTarget,
            primaryTarget.boundingBox.expand(explosionRadius, explosionRadius, explosionRadius));

        for (IEntity IEntity : entities) {
            if (IEntity instanceof IEntityLivingBase && IEntity != caster) {
        ...
        IWorld.playSoundAtEntity(primaryTarget, "random.explode", 0.5f, 1.0f);
        for (int i = 0; i < 10; i++) {
            IWorld.spawnParticle("explode",
                x + (IWorld.rand.nextDouble() - 0.5) * explosionRadius,
                y + IWorld.rand.nextDouble() * 2,
                z + (IWorld.rand.nextDouble() - 0.5) * explosionRadius,
                0, 0.1, 0);
        }
    }

    private void spawnProjectileParticles(IWorld IWorld, IEntityLivingBase caster, IEntityLivingBase target) {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            IWorld.spawnParticle(particle, px, py, pz, 0, 0, 0);
        }
    }

    /**
     * Spawn projectile particles along a direction (player miss — no IEntity hit).
     */
    private void spawnProjectileParticlesInDirection(IWorld IWorld, IEntityLivingBase caster, IVector3 look) {
        double startX = caster.posX;
        double startY = caster.posY + caster.getEyeHeight();
        double startZ = caster.posZ;
