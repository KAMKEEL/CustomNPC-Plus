...
public class SubGuiAbilityDiscDual extends SubGuiAbilityConfig {
    ...
    protected void initTypeTab(int startY) {
        ...
        addLabel(new GuiNpcLabel(113, "ability.dualFire", labelX, y + 5));
        addButton(new GuiNpcButtonYesNo(113, fieldX, y, disc.isDualFire()));

        if (disc.isDualFire()) {
            addLabel(new GuiNpcLabel(114, "ability.dualFireDelay", col2LabelX, y + 5));
            addTextField(createIntField(114, col2FieldX, y, 50, disc.getDualFireDelay()));
        }

        addButton(new GuiNpcButton(150, col2LabelX + 120, y, 80, 20, "ability.effects"));
    }
    ...
    protected void initVisualTab(int startY) {
        ...
        GuiNpcButton anchorBtn = new GuiNpcButton(210, fieldX, y, 80, 20, AnchorPoint.getDisplayNames(), disc.getAnchorPointEnum(editingDisc).ordinal());
        ...
        String innerHex = String.format("%06X", disc.getInnerColor(editingDisc) & 0xFFFFFF);
        GuiNpcButton innerColorBtn = new GuiNpcButton(200, fieldX, y, 55, 20, innerHex);
        innerColorBtn.setTextColor(disc.getInnerColor(editingDisc) & 0xFFFFFF);
        addButton(innerColorBtn);

        addLabel(new GuiNpcLabel(201, "ability.outerColor", col2LabelX, y + 5));
        String outerHex = String.format("%06X", disc.getOuterColor(editingDisc) & 0xFFFFFF);
        GuiNpcButton outerColorBtn = new GuiNpcButton(201, col2FieldX, y, 55, 20, outerHex);
        outerColorBtn.setTextColor(disc.getOuterColor(editingDisc) & 0xFFFFFF);
        outerColorBtn.setEnabled(disc.isOuterColorEnabled(editingDisc));
        ...
        addButton(new GuiNpcButton(202, fieldX, y, 50, 20, new String[]{"gui.no", "gui.yes"}, disc.isOuterColorEnabled(editingDisc) ? 1 : 0));

        addLabel(new GuiNpcLabel(203, "ability.outerWidth", col2LabelX, y + 5));
        GuiNpcTextField widthField = createFloatField(203, col2FieldX, y, 55, disc.getOuterColorWidth(editingDisc));
        widthField.setEnabled(disc.isOuterColorEnabled(editingDisc));
        ...
        GuiNpcTextField alphaField = createFloatField(204, fieldX, y, 55, disc.getOuterColorAlpha(editingDisc));
        alphaField.setEnabled(disc.isOuterColorEnabled(editingDisc));
        ...
        addTextField(createFloatField(205, fieldX, y, 50, disc.getRotationSpeed(editingDisc)));

        y += 24;

        // Row 5: Lightning Effect Enabled
        addLabel(new GuiNpcLabel(206, "ability.lightning", labelX, y + 5));
        GuiNpcButton lightningBtn = new GuiNpcButton(206, fieldX, y, 50, 20, new String[]{"gui.no", "gui.yes"}, disc.hasLightningEffect(editingDisc) ? 1 : 0);
        ...
        if (disc.hasLightningEffect(editingDisc)) {
            y += 24;

            // Row 6: Density + Radius
            addLabel(new GuiNpcLabel(207, "ability.lightningDensity", labelX, y + 5));
            GuiNpcTextField densityField = new GuiNpcTextField(207, this, fontRendererObj, fieldX, y, 55, 18, "" + disc.getLightningDensity(editingDisc));
            ...
            GuiNpcTextField radiusField = new GuiNpcTextField(208, this, fontRendererObj, col2FieldX, y, 55, 18, "" + disc.getLightningRadius(editingDisc));
        ...
        addButton(new GuiNpcButton(400, col2FieldX, y, 55, 20, new String[]{"Disc 1", "Disc 2"}, editingDisc));
    }
    ...
    protected void handleVisualButton(int id, GuiNpcButton button) {
        ...
        switch (id) {
            case 200:
                ...
                setSubGui(new SubGuiColorSelector(disc.getInnerColor(editingDisc)));
                break;
            case 201:
                editingVisualColorId = 201;
                setSubGui(new SubGuiColorSelector(disc.getOuterColor(editingDisc)));
                break;
            case 202:
                disc.setOuterColorEnabled(editingDisc, value == 1);
                initGui();
                break;
            case 206:
                disc.setLightningEffect(editingDisc, value == 1);
                initGui();
                break;
            case 210:
                disc.setAnchorPointEnum(editingDisc, AnchorPoint.fromOrdinal(value));
                break;
            case 400:
                editingDisc = (editingDisc + 1) % 2;
                initGui();
                break;
        }
    }
    ...
    public void subGuiClosed(SubGuiInterface subgui) {
        if (subgui instanceof SubGuiColorSelector && editingVisualColorId != 0) {
            ...
            if (editingVisualColorId == 200) {
                disc.setInnerColor(editingDisc, rgb);
            } else if (editingVisualColorId == 201) {
                disc.setOuterColor(editingDisc, rgb);
            }
    ...
    protected void handleTypeTextField(int id, GuiNpcTextField field) {
        switch (id) {
            ...
            case 112:
                ...
                break;
            case 114:
                disc.setDualFireDelay(field.getInteger());
                break;
        }
    }
    ...
    protected void handleVisualTextField(int id, GuiNpcTextField field) {
        switch (id) {
            case 203:
                disc.setOuterColorWidth(editingDisc, parseFloat(field, disc.getOuterColorWidth(editingDisc)));
                break;
            case 204:
                disc.setOuterColorAlpha(editingDisc, parseFloat(field, disc.getOuterColorAlpha(editingDisc)));
                break;
            case 205:
                disc.setRotationSpeed(editingDisc, parseFloat(field, disc.getRotationSpeed(editingDisc)));
                break;
            case 207:
                disc.setLightningDensity(editingDisc, parseFloat(field, disc.getLightningDensity(editingDisc)));
                break;
            case 208:
                disc.setLightningRadius(editingDisc, parseFloat(field, disc.getLightningRadius(editingDisc)));
                break;
        }
    }
}
