...
public final class NPCMountUtil {
    ...
    public static boolean handleMountedMovement(EntityNPCInterface npc, MountState state, float strafe, float forward) {
        ...
        IAttributeInstance moveAttribute = npc.getEntityAttribute(SharedMonsterAttributes.movementSpeed);
        if (moveAttribute != null) {
            moveAttribute.setBaseValue(getMountMoveSpeed(npc));
        }
        ...
        float moveSpeed = moveAttribute != null ? (float) moveAttribute.getAttributeValue() : getMountMoveSpeed(npc);
        ...
        npc.stepHeight = 1.0F;
        npc.jumpMovementFactor = Math.max(0.02F, moveSpeed * 0.1F);
        ...
        syncRiderMotionWithMount(npc, rider);
    ...
    private static void syncRiderMotionWithMount(EntityNPCInterface npc, EntityPlayer rider) {
        if (rider == null) {
            return;
        }
        ...
        if (!rider.worldObj.isRemote && (Math.abs(previousMotionX - rider.motionX) > 1.0E-4D
                || Math.abs(previousMotionY - rider.motionY) > 1.0E-4D
                || Math.abs(previousMotionZ - rider.motionZ) > 1.0E-4D)) {
