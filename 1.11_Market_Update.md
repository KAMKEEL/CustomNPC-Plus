...
| Blacklist check | `createListing()` | `AuctionBlacklist.isBlacklistedForPlayer()` check |
| Bound item check | `createListing()` | Auto-blocks Soulbound/Profile-Slotbound items |
...
### 3.8 Item Blacklist System

Prevents specific items from being listed on the auction house.

#### Blacklist Types

| Type | Description | Example |
|------|-------------|---------|
| **Items** | Block specific items by registry name | `minecraft:diamond_sword` |
| **Wildcards** | Block items matching pattern | `minecraft:diamond*` |
| **Mods** | Block all items from a mod | `customnpcs` |
| **NBT Tags** | Block items with specific NBT | `ench` (enchanted items) |

#### Auto-Blocked Items (Always Blocked)

These items are **always** blocked regardless of blacklist settings:
- **Soulbound Items** - Items with `cnpc_soulbind` requirement
- **Profile-Slotbound Items** - Items with `cnpc_profile_slot` requirement

The system checks for these under `RPGCore/Requirements` in the item's NBT.

#### Bypass Permission

Players with OP status or the `customnpcs.auction.blacklist.bypass` permission can list blacklisted items.

#### Configuration

```
Market.Auction.Blacklist:
  Enabled: true
  BlacklistedItems: ["minecraft:barrier", "minecraft:command_block*"]
  BlacklistedMods: []
  BlacklistedNBTTags: []
```
...
| Command | Description |
|---------|-------------|
| `/cnpc auction blacklist add <item\|mod\|nbt> <value>` | Add to blacklist (runtime only) |
| `/cnpc auction blacklist remove <item\|mod\|nbt> <value>` | Remove from blacklist (runtime only) |
| `/cnpc auction blacklist list [item\|mod\|nbt]` | List blacklisted entries |
| `/cnpc auction blacklist reload` | Reload blacklist from config |
| `/cnpc auction blacklist check` | Check if held item is blacklisted |
...
controllers/data/
  ...
  AuctionClaim.java          ✅
  AuctionFilter.java         ✅
  AuctionBlacklist.java      ✅ (item blacklist checking + bypass)
...
```
command/
  AuctionCommand.java                    ✅ /cnpc auction with list, view, open, blacklist
  command/auction/
    BlacklistSubCommand.java             ✅ /cnpc auction blacklist subcommands
```
...
// =========================================
// Auction Blacklist (Market.Auction.Blacklist)
// =========================================
public static boolean BlacklistEnabled = true;
public static String[] BlacklistedItems = {};    // Registry names (supports wildcards)
public static String[] BlacklistedMods = {};     // Mod IDs to block entirely
public static String[] BlacklistedNBTTags = {};  // NBT tag keys to block
```
...
**Blacklist Bypass:**
- OP players or those with `customnpcs.auction.blacklist.bypass` can list blacklisted items
- Soulbound and Profile-Slotbound items are **always** blocked (no bypass)

---

## 7. Commands

### `/cnpc auction` Command

Allows interaction with the Auction House without needing an Auctioneer NPC.

| Subcommand | Usage | Permission | Description |
|------------|-------|------------|-------------|
| `list` | `/cnpc auction list [page]` | OP | View active auctions in chat |
| `view` | `/cnpc auction view <player>` | OP | View a player's auction activity |
| `open` | `/cnpc auction open` | Any | Opens the Auction House GUI |

**Examples:**
```
/cnpc auction list           - Show first page of active auctions
/cnpc auction list 2         - Show page 2 of auctions
/cnpc auction view Steve     - Show Steve's listings, bids, and claims
/cnpc auction open           - Open the full Auction House GUI
```

### `/cnpc auction blacklist` Command

Manage the auction item blacklist (runtime changes, not persisted to config).

| Subcommand | Usage | Permission | Description |
|------------|-------|------------|-------------|
| `add` | `/cnpc auction blacklist add <item\|mod\|nbt> <value>` | OP | Add to blacklist |
| `remove` | `/cnpc auction blacklist remove <item\|mod\|nbt> <value>` | OP | Remove from blacklist |
| `list` | `/cnpc auction blacklist list [item\|mod\|nbt]` | OP | List blacklisted entries |
| `reload` | `/cnpc auction blacklist reload` | OP | Reload from config |
| `check` | `/cnpc auction blacklist check` | Any | Check if held item is blacklisted |

**Examples:**
```
/cnpc auction blacklist add item minecraft:diamond_sword
/cnpc auction blacklist add item minecraft:diamond*    (wildcard)
/cnpc auction blacklist add mod industrialcraft2
/cnpc auction blacklist add nbt ench
/cnpc auction blacklist remove item minecraft:diamond_sword
/cnpc auction blacklist list item
/cnpc auction blacklist check                          (checks held item)
```
...
- [x] **Item Blacklist System** - Block items by registry name, mod, NBT tag, or wildcard pattern
  - Auto-blocks Soulbound and Profile-Slotbound items (checks `RPGCore/Requirements`)
  - Per-player bypass via `customnpcs.auction.blacklist.bypass` permission
  - Runtime add/remove commands (not persisted to config)
  - Check command to test held item
...
| Item Blacklist | Static utility class | Simple, fast checks without instantiation; auto-blocks bound items |
| Bound Item Detection | Check `RPGCore/Requirements` NBT path | Uses AttributeItemUtil constants for consistency |
